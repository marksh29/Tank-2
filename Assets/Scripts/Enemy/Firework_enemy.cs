...
public class Firework_enemy : MonoBehaviour
{
    [SerializeField] Animator anim;
    [SerializeField] GameObject player, fire_position;
    [SerializeField] float[] min_max_pos;
    [SerializeField] float speed, fire_timer, move_timer, attack_distance, damage;
    public bool stay, dead, end;
    ...
    void Start()
    {
        damage = Player_stats.Instance.enemy_firework_power;
    }
    void Update()
    {
        if (!end && !dead && transform.position.z - player.transform.position.z < Player_stats.Instance.enemy_distance)
        {
            if(!stay)
            {
                ...
                if (fire_timer <= 0 && (transform.position.z - player.transform.position.z < Player_stats.Instance.attack_distance))
                {
                    fire_timer = Player_stats.Instance.enemy_fire_time;
                    StopAllCoroutines();
                    StartCoroutine(Fire_on(3f));
                }                    
            }
            else
            {
                if (transform.position.z - player.transform.position.z <= attack_distance)
                {
                    Vector3 targetDirection = new Vector3(player.transform.position.x, transform.position.y, player.transform.position.z) - gameObject.transform.position;
                    ...
                    Player_hp.Instance.Damage(damage * Time.deltaTime);
                }
            }           
        }
        //else
    ...
    IEnumerator Fire_on(float time)
    {
        anim.SetTrigger("fire");
        stay = true;
        fire_position.SetActive(true);
